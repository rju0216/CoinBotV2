...
5. **활성 코드 위치**:
   ...
   - `src/research/` — **모델 개발 오프라인 substrate**: data(로더·audit)·validation
     ...
     **D2 baseline 박제**(D-042)·**완전 kill 0 유지**(D-043, PARK 정량 재소환 조건).
     발견: θ knife-edge=표집아티팩트 · **금지가정 "방향편향" 실측**(예측 up 77.7% vs 라벨 32.7%,
     |편향|↔적중률 상관 −0.739). ※ "알파 양방향 실재" 서술은 **I-014(모집단 불일치)로 재계산 대기 —
     인용 금지**.
