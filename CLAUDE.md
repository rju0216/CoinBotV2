...
20. **소요 시간은 판단 근거가 아니라 참고 정보**: 테스트·백테·재학습의 **예상 소요 시간을 이유로 항목을 건너뛰거나 축소·유보 제안하지 않는다**. 시간은 **참고 정보로만 명시**하고 실행 여부는 사용자가 판단한다(수십 시간 규모면 그때 별도 고려). 커버리지를 넓히는 쪽이 기본값이다 — "가능성을 최대한 확인"이 우선. **단 방법론적 이유로 유보를 제안하는 것은 계속 유효하다**(과적합·winner's curse·사전등록 없는 다중비교·오염 baseline 위 축적 등). 그 경우 **시간이 아니라 그 방법론적 이유를 근거로** 제시하고, 가능하면 **사전등록 후 전수 실행**처럼 커버리지와 규율을 동시에 만족하는 설계를 낸다.
