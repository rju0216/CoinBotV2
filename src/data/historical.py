"""과거 캔들 로더 — 백테스트용 캐시(CSV)·API 병합 (동기)."""
...
class HistoricalDataLoader:
    ...
    def download_to_csv(self, timeframe: str, limit: int = 300) -> str:
        df = self.download(timeframe, limit)
    ...
    def download_range_merged(
        ...
        for r_start, r_end in ranges_to_fetch:
            ...
            chunk = self.download(timeframe, limit=n_bars, since=r_start)
            if not chunk.empty:
