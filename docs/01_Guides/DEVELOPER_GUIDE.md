...
#### ML/DL 기반 plugin 의 train_meta

학습된 모델을 사용하는 plugin (ml_*, dl_*, ensemble) 의 경우 `params["model_path"]`
밑에 `train_meta.json` 을 두면 `extract_train_meta` (§4.3) 가 학습 cutoff 와
OOS Acc 를 자동 인식해 OOS warm-up baseline 으로 사용한다 (BL-2-3 hotfix-A).
single-model plugin 은 base 클래스 default 동작으로 충분하며, multi-model
strategy (ensemble 등) 만 sub-plugin 집계 형태로 override 한다.
...
| `extract_train_meta()` | 라이브 시작 시 OOS warm-up baseline 산정 (BL-2-3 hotfix-A) | `(cutoff_dt, learned_oos_acc)` 반환. base 클래스 default 가 `params["model_path"]/train_meta.json` 자동 인식. multi-model strategy (ensemble) 만 sub-plugin 집계 override 권장 |
...
class Signal:
    ...
    meta: dict = field(default_factory=dict)  # 진단 정보
```

`meta` 는 단순 자유 dict 가 아니라 **라이브 운영 진단 컨벤션** 을 따른다
(BL-2-3 hotfix-F). 새 plugin 작성 시 동일 키를 사용하면 라이브 운영에서
sub-plugin 추론 실패 분석이 일관된다.

| 키 | 셋팅 위치 | 의미 |
|---|---|---|
| `fail_reason` | `features.py` (`"all_features_nan"` / `"dropna_lt_lookback"`) + plugin 자체 (`"feature_mismatch"` / `"sequence_invalid"` 등) | 추론 실패 사유. HOLD 신호의 conf=1.00 이 "확률 100% HOLD 확신" 인지 "추론 실패" 인지 구분용 |
| `nan_by_tf` | `features.py` `_group_nan_by_tf` | NaN 컬럼을 timeframe (1d/4h/15m 등) 별로 분류. 어느 TF 의 데이터가 부족한지 즉시 식별 |
| `gap_to_latest`, `used_row_ts` | `features.py` (정상 path) | 정상 추론 시 사용된 features 행 메타. 라이브-백테 cycle 일관성 확인용 (§5.5) |

ensemble 등 multi-model strategy 는 sub-plugin 의 `meta` 를 집계해
`{name: {"reason": ..., "nan_by_tf": ..., "available_rows": ...}}` 형태로 보관
(`src/strategy/plugins/ensemble.py` 참조).
...
**백테/라이브 공통** (`AbstractEngine.evaluate_strategies_on_bar`):
...
**라이브 전용 추가 단계** (`CoreEngine._on_bar_closed`):

```
BAR_CLOSED 이벤트
  ↓
0-a) data_store.append_candle (DB 누적 기록)
0-b) circuit_breaker 감시 — broker.executor.cb.is_open이면 EventBus publish (BL-2-1)
0-c) _should_process_bar(tf, ts_ms) — ccxt가 봉 진행 중에도 _on_bar_closed를
     트리거하므로 같은 tf + ts 재호출이면 즉시 return (호가창 fetch보다 앞.
     I-BL005 회귀 방지)
0-d) [master_tf만] orderbook_collector.fetch_and_save (BL-2-2)
0-e) [master_tf + position 보유 + live] broker.get_position() 사전 동기화 —
     거래소 ∅ 인지 시 _sync_unexpected_close 호출 후 SL/TP 캔들 검사 skip
     (I-BL015 외부 청산 자동 인지: SL/TP spike, manual close, 강제 청산)
1~4) 공통 흐름 (check_candle_sl_tp → strategy_exits → evaluate → equity)
5-a) [master_tf + 보유 시] _log_position_status (BL-2-3 hotfix-E)
5-b) [master_tf] _log_account_status (BL-2-4 hotfix-G, 매 주기 재정 가시화)
5-c) oos_monitor.evaluate_pending (BP-2-3, horizon 도달 prediction 채점)
```

라이브 추가 단계의 운영 효과(자동 인지·복구 표, 모니터링 로그 형식)는
`USER_GUIDE.md` §6.5 / §6.6 참조.
...
`AbstractEngine.close_position` 공통 흐름:

1. `broker.cancel_all_orders()` — pending SL/TP 취소
2. **`broker.close_position(side, size, fill_price)` — 시장가 청산** (강건성 처리):
   - 라이브에서 `LiveExecutor.close_position` 은 사전 `get_position()` 검사로
     거래소 ∅ 시 redundant 호출 skip (I-BL010 — 거래소가 SL/TP 자동 청산한
     직후 봉 마감 redundant close 시 OKX 51169 reject 차단)
...
라이브 엔진 시작 시 `_restore_state` 가 거래소·DB 상태를 매칭해 포지션을
복원한다. 시퀀스:

1. 잔액·peak_equity 복원 → `RiskManager.set_initial_balance` + `update_equity`
2. `broker.get_position()` + `data_store.get_open_trades()` 비교
3. case 분기 (코드 흐름):

| 거래소 | DB | 처리 |
|---|---|---|
| ∅ | ∅ | clean startup, return |
| ∅ | O | 각 open trade 마다 `_fetch_actual_exit` 호출 (I-BL013) — `fetch_closed_orders` 에서 reduceOnly + 반대 방향 closed order 찾아 정확한 exit_price/pnl/reason 복원. 실패 시 SL 가격 추정 fallback + WARNING |
| O | 0 strategy | RuntimeError 중단 (정책 7 (a) — 전략 없는 뼈대 상태) |
| O | DB 매칭 | `_match_trade_to_exchange` (side + size) → 입양 |
| O | DB 매칭 실패 | strategy_name="_unknown" ORPHAN |

4. 입양된 포지션의 strategy_name이 `strategy_by_name` 에 있으면 OPEN, 없으면
   ORPHAN (전략 hook `update_stop_loss`/`should_force_exit` 호출 안 됨, 엔진
   레벨 SL/TP 만 유효)
5. **`_verify_and_restore_sl_tp`** (live + 포지션 복원 시, I-BL011) —
   `fetch_open_orders` 로 거래소 conditional order 검증. OKX `slTriggerPx`/
   `tpTriggerPx` 기준 허용 오차 0.1%. 누락 시 `place_stop_loss` /
   `place_take_profit` 자동 재등록

운영 시점의 입양 결과 표(사용자 관점) 는 `USER_GUIDE.md` §6.2 참조.

### 5.5 학습-추론 cycle 일관성 (`features.py`, BL-2-3 hotfix-F)

ML/DL plugin 의 라이브 추론이 백테와 같은 features 행을 참조해야 한다 — 그렇지
않으면 동일 시점·동일 데이터에 다른 신호가 발생할 수 있다 (I-BL007). 일관성을
보장하는 핵심 규칙 3개:

1. **sub_tf 진행 중 봉 제외** (`_is_in_progress_bar`): multi-TF features 계산 시
   sub_tf 의 마지막 봉이 `now` 이전에 마감되지 않았으면 그 행을 drop.
   (`sub_close_ms = sub_last_ts.value/10**6 + sub_tf_ms`)
2. **`get_features_for_ctx` 의 `ts < now` 필터** (Phase 3-D): 라이브 path 도
   백테와 동일하게 `now` 보다 작은 ts 행만 사용. 봉 t+1 시작 시점의 진행 중
   봉을 features 에 포함하지 않도록.
3. **dropna 패턴 통일**: `get_clean_last_row` (ML 1행) / `get_clean_features_for_sequence`
   (DL 시퀀스) helper 가 양쪽 모두 동일 dropna 규칙 적용.

라이브 추론 실패 시 `Signal.meta` 에 진단 정보가 채워진다(§4.5):
- `fail_reason`: `"all_features_nan"` / `"dropna_lt_lookback"` 등
- `nan_by_tf`: NaN 컬럼의 timeframe별 분류 (어느 TF의 데이터가 부족한지 식별)

새 plugin 작성 시 이 helper 들을 사용하면 cycle 일관성이 자동 보장된다.
직접 dropna 를 호출하면 sub_tf 진행 중 봉 처리 누락 위험.
...
| 새 알림 채널 | `src/utils/notifier.py` 에 `Notifier` 추상 상속 클래스 추가 + `build_notifier_from_config` 의 channels 분기 추가 | config `live.notifications.channels` 리스트에 키 등록. 자격증명 미설정 시 `LogNotifier` 자동 fallback 패턴 따를 것 (Telegram/Email 참조). `parse_mode` 같은 채널-specific 포맷팅은 plain text 우선 (I-BL014 회귀 방지) |
| 새 거래소 | `src/execution/` 에 신규 executor + `Broker` 분기 | `LiveExecutor` 패턴 준수 권장: ① `CircuitBreaker` + `_retry_api` 모듈 함수 활용 (인스턴스 메서드 자기 참조 금지 — I-BL008 회귀 방지). ② `set_margin_mode` 거래소별 필수 파라미터 (OKX `lever` 등) 인지. ③ `close_position` 사전 `get_position()` 검사로 redundant 호출 skip (I-BL010). ④ conditional order (SL/TP) 가 있다면 `_verify_and_restore_sl_tp` 와 호환되는 식별 키 (OKX `slTriggerPx`/`tpTriggerPx` 등) 노출. EXTENSION (BLE-1) 다중 거래소 진입 시 본 패턴 기반 |
...
라이브 운영 중 신호·체결 이상이 보일 때 점검 순서 (BL-2-3/4 hotfix 의 진단 인프라 활용):

1. **봉 진행 중/마감 구분** — `_on_bar_closed` 가 한 봉 동안 여러 번 트리거되는 게 정상이다. `_should_process_bar` 가 같은 ts 재호출을 차단 (§5.1 0-c). 이 검사가 호가창 fetch 보다 앞에 있어야 하므로 (I-BL005), `_on_bar_closed` 본문 수정 시 단계 순서 보존 필수.

2. **sub-plugin 추론 실패 진단** — ensemble HOLD 의 `conf=1.00` 이 의심스러우면 신호 로그의 `meta.contributors`/`failures` 확인 (§4.5). features 부족이면 `nan_by_tf` 가 timeframe별 결손을 가리킴. 새 plugin 도 `Signal.meta` 에 `fail_reason`/`nan_by_tf` 일관 셋팅 권장.

3. **외부 청산 인지 확인** — 봉 OHLC 밖 SL/TP spike 청산이 발생하면 `_sync_unexpected_close` 로그 (`Position closed externally — syncing`) 와 EXIT 텔레그램 도달 여부 확인. 둘 다 누락이면 `broker.get_position()` 또는 `fetch_closed_orders` 자체에 문제 있을 수 있음.

4. **재시작 복원 결과 확인** — `_restore_state` 직후 `Restored position: ...` 로그의 `status=ORPHAN` 여부, `_verify_and_restore_sl_tp` 의 `SL/TP conditional orders verified alive` 또는 `re-registering` 로그 확인 (§5.4).

5. **Warmup 가속 (`_features_cache`)** — OOS warm-up 이 매 ts 마다 81 피처를 재계산하면 1시간 이상 소요 (I-BL004). `_features_cache` 임시 주입 + `try/finally` cleanup 패턴이 적용되어 1분 30초대로 단축됨. warm-up 시간이 길어졌다면 이 패턴이 깨졌는지 점검.

라이브 운영 절차·자동 인지·텔레그램 알림 종류 등 사용자 관점은 `USER_GUIDE.md` §6 참조.
