...
| D4 | 구현 (…D4-4 ✅ t emission / **D4-5 진행중: S0-S3 ✅**(사전등록·EM 11x·K G5/t4·Dev 계수확정) / **S4 ✅ OOS 무매매 발견**(고정 τ 과적합 I-017) → **S4-R τ 재설계**(O-10 나) 진행 / S5-S7 예정) | 진행중 | 2026-07-01~ | D4-3 b14f175 / D4-3.5 e1135c8 / D4-4 21c1f19 / D4-5 S0 493d314·S1 54e6f3a·S2 a311a31·S3 d7cd3f2 |
...
### D4-5 — OOS 정직 평가 + emission 표준 확정 + as-built 스펙 (진행중)

> gen1(추세-단독 t-or-Gaussian HMM)의 **정직한 OOS 성능 확정**과 **미결 표준(O-6 emission·
> O-8 fit) 확정**, **gen1 as-built 스펙 작성**이 목표. 백테 대량 실행을 수반하므로 **방법론
> 오염(OOS·seed selection)을 막는 사전등록을 먼저 못 박고**(S0) 착수한다.

#### 방법론 대전제 — 3계층 검증 (D3 Step0 계승, D4-5 전체 강제)
| 계층 | 대상 | 데이터 | 규칙 |
|---|---|---|---|
| 튜닝 | K·τ·계수 스윕 | **Dev 2020-01~2022-12** | OOS 절대 미열람 |
| 정직 평가 | 확정값 1회 평가 | **OOS 2023-01~2026-06** | 무튜닝·사후 계수 변경 금지 |
| 스트레스 | 반응 점검(정성) | **2018 홀드아웃**(Binance 현물) | 학습·튜닝 미사용 |

#### S0 — OOS 사전등록 프로토콜 (pre-registration, 착수 전 고정)
착수 후 결과를 보고 기준을 바꾸는 것을 금지하기 위해, 아래를 **S1 시작 전에 고정**한다.

**(P-1) 1차 평가 지표 (사전 정의)**
- 주 지표 = **위험조정 수익 Calmar-유사 = OOS total_pnl% / |max_dd%|**. 보조 = PF·승률·손익비·거래수·평균보유봉.
- 모든 백테는 **정합성 게이트(규칙10)** 통과 필수: `trades pnl 합 = metrics.total_pnl = equity 변화량`. 불일치 시 결과 폐기·이슈 등록.

**(P-2) emission 표준 G vs t 선택 규칙 (O-6·O-8) — 2지선다 오염 최소화 + 노이즈 방어**
- Dev에서 G·t **각각** 계수·K를 독립 최적화 → 그 확정본으로 **OOS 평가**(seed 앙상블).
- 채택 규칙(사전 고정) — **t 는 아래 3조건 모두 충족 시에만 채택, 그 외 Gaussian(단순성)**:
  - **(a) 효과크기**: OOS Calmar 앙상블 평균 **≥ 10% 상대 우위**(오컴 최소 문턱).
  - **(b) 분포 분리(노이즈 초과)**: **t 평균 − 1 SD > G 평균** (두 seed 분포 실질 비겹침 → I-012 seed 노이즈 방어).
  - **(c) 이론(스트레스)**: **2018 max_dd 개선(또는 동등)** — fat-tail 이점의 이론적 자리(스펙§1.4).
- **보조 안전장치**: P-1 보조지표(PF·pnl%·승률)가 Calmar 판정과 **역방향이면 보류·재검토**(Calmar 단독 뒤집기 금지 — max_dd 단일 에피소드 I-004 민감성).
- **seed 앙상블 n ≥ 8** (SD 추정 안정 위해; I-007 최적화 선결이라 S1 우선).
- 근거: t는 상태별 ν 로 파라미터가 많아 "유의미 우위"의 입증 책임을 t가 진다. (a)만으론 노이즈에 속고 (b)만으론 n 불안정 → 결합. fat-tail 은 특히 2018 패닉에서 나타나야 하므로 (c) 필수.

**(P-3) seed 오염 방지 절차 (I-012)**
- fit 표준(n_init·seed 방침)은 **Dev의 seed 간 안정성·LL**로만 확정. **인샘플·OOS P&L 로 seed 선택 금지**(과적합).
- OOS 평가는 그 방침의 **대표 seed(seed=0) + seed 앙상블(n≥8, P-2 (b) SD 산출용) 산포 병기**. seed 산포는 "선택 대상"이 아니라 "안정성 보고 대상".

**(P-4) 계수 스윕 비용 분리 (I-007 직결)**
- **재빌드 축(느림, walk_forward 재적합)**: `τ·emission·K·retrain-months·seed` → 후보를 **좁게**(τ 3~4개, retrain 2개 등).
- **백테-only 축(빠름, artifact 고정 dict 오버라이드)**: `k_trend_sl·k_trend_trail·cooldown·theta_trend·risk%` → **넓게** 그리드.
- 계수는 **안정 구간으로 정당화**(스펙§6, 단일 매직넘버 금지 — 인접 계수에서 순위·부호 안정 확인).

**(P-5) 2018 스트레스 격 한정 (결정 A=(가) Binance 현물)**
- 무기한 학습 artifact를 **현물 2018 패닉에 역적용**해 반응 관찰. **정성 점검**(정량 판정 아님).
- 해석 시 명시: ① 소스 상이(현물≠무기한, funding/베이시스 부재) ② 76봉 갭(0.4%)의 ATR/윈도우 영향.

**(P-6) 재현성 게이트**: 스레드=1(OMP/MKL/OPENBLAS/NUMEXPR) + `default_rng(seed)` → 결정론(D4-4 확립).

**(결정 B=(나)) 백테 주체 = Claude 직접** — 원칙(백테 정책=사용자 직접)의 **D4-5 한정 명시 승인 예외**
(D4-4와 동일). programmatic override로 `config/default.yaml` **무접촉**(규칙7).

#### 단계 구성
| 단계 | 내용 | 게이트 |
|---|---|---|
| **S0** | 사전등록(P-1~P-6)·단계구성 문서화·백테주체 예외 기록 | 사용자 승인 |
| **S1** | **I-007 EM 성능 최적화** (벡터화 한정 — 수치 동일성 보존) | 참조 Baum-Welch **atol 1e-10 유지**(절대 게이트) + 빌드 벤치 |
| **S2** | K선택 **t 배선**(I-013·`make_emission`) + `CORE_COMBOS`→`{(TREND,LONG),(TREND,SHORT)}` 정리 + Dev 커버리지 진단 → **emission별 최종 K** | 진단표·회귀 |
| **S3** | **Dev 계수 스윕**(G·t 각각, P-4 2단계: 백테-only 넓게/재빌드 좁게) → emission별 확정 계수 | 안정구간 정당화 |
| **S4** | **OOS 정직 평가**(S3 확정값 1회) + I-012 다중seed(P-3) — **결과: OOS 전구간 무매매**(14윈도우 전부 \|μ\|/σ max 0.03~0.05 < τ → trend 상태 0). 정직평가가 **고정 절대 τ 과적합 포착**(I-017). 정합성 20/20 | ✅ 발견 |
| **S4-R** | **τ 재설계**(gen1 재설계 = O-10 (나)): R1 원인규명(anchored train 확장 희석 vs 시장변화) → R2 방향(τ상대화/rolling/하이브리드) → R3 구현 → R4 Dev 재튜닝 → R5 OOS 재평가. **오염 최소화**(Dev·구조 논리·OOS 최종1회) | R1 진단 착수 |
| **S5** | **2018 스트레스**(P-5 정성) | 반응 점검 |
| **S6** | **emission 표준 확정**(O-6·O-8, P-2 규칙 적용) + fit 표준 확정 | 사전등록 기준 적용 |
| **S7** | **gen1 as-built 스펙** 작성(`docs/01_Guide_Docs/`) + config 최종 반영 + 보고서 갱신 + 커밋 | 문서↔코드 양방향(규칙13) |

- 각 Step 종착 시 메모리 임시 기록(규칙9), Step별 독립 fresh-eyes(규칙14), 착수 직전 구조 점검(규칙8).
- **잠정 상태**: I-015(`nu_min` 2.0→2.1 상향)는 S1/S2 t 배선 시 함께 검토.
...
| I-017 | **고정 절대 τ 스케일 과적합**: S3 확정 τ(G 0.07·t 0.05)로 OOS(2023-2026) walk-forward 20 artifact(10seed×2em) 백테 **전구간 무매매**. 원인=OOS 전 윈도우 \|μ\|/σ max(0.03~0.05) < τ → trend 상태 0. Dev \|μ\|/σ max(~0.10)→OOS(~0.05) 스케일 이동 | D4-5 S4 | OPEN | **정직평가(P-1)가 τ 과적합 포착**(OOS 보고 τ 조정=P-1 위반 금지). 해결=**S4-R τ 재설계**(O-10 (나)). 원인 A(시장변화)/B(anchored train 확장 희석) S4-R1 규명 → τ상대화 or rolling walk-forward. 스펙 O-1(고정τ) 한계·gen2 G2-4(τ적응) 예견 실증 |
...
| O-10 | gen1 τ OOS 무매매(I-017) 처리 | **(나) gen1 내 τ 재설계** (단순 실패 결론 거부) — 고정 절대 τ = 명백한 설계결함(\|μ\|/σ 스케일이 regime마다 변동)이라 구조 수정 정당. **S4-R**: R1 원인규명 → R2 방향(τ상대화 분위수 / rolling walk-forward / 하이브리드) → R3~R5. **오염 최소화**: 재설계는 Dev·구조 논리로만(OOS 성능 목표 금지), OOS 최종 1회. OOS 인지 후 재설계 = 약한 오염 명시·수용 | 진행 D4-5 S4-R |
