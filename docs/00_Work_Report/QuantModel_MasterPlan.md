...
| **7 layer 0~2 개선** | 표본/용량 가설 검정(E-배치)·배치 진단(편향·s/q 분해·피처 방향성·알파 3분해)·갈래×config 확정·처방 실행·**layer-3 재측정** | **진전 바(D-046)**: D2 초과 + 연도별 **선택알파** 전부 양수 + 지평 패밀리 선택알파 부호 일치 | — |
...
| **F-20** | (fresh-eyes Phase7) **`alpha_decomp.barrier_matched_long_returns` 가 캔들 감사 없이 위치 오프셋을 쓴다** — `pos + horizon_bars` 는 **위치** 기준이라 캔들에 결손이 있으면 "N봉 뒤"가 시간상 N봉 뒤가 아니다. 호출부가 `load_audited` 대신 `pd.read_csv` 로 읽어 **규칙 16(표준 진입점) 우회**. 온그리드 가드·간격 검증 없음. I-001(1d 캔들 손상이 audit 미검출 클래스) 전례가 있다. | fresh-eyes Phase7 | 열림(MEDIUM). 현 사용 캔들(15m/1h/4h)은 audit_continuity coverage≥0.9999 로 확인된 계열이라 미발현으로 추정되나 **검증 안 됨**. Step 7.5c 분석 전 load_audited 경유로 교체하거나 최소한 간격 검증 추가 권고. |
| **F-21** | (fresh-eyes Phase7, 저심각도 묶음) ① `feature_audit.conditional_distribution` 의 `std_mean_diff` 분모가 within-group SD 가 아닌 **전체 SD**(expire·NaN 라벨 포함) → Cohen's d 가 아니며 이름·정의가 어긋남 ② `alpha_decomp.ev_expire_gate` 에 **θ 인자 없음** → θ>0 셀을 재현하려는 호출자는 조용히 **더 넓은 모집단**을 얻음(현 D2 는 θ=0 이라 무해) ③ `policy_eval._max_consecutive_losses` 가 **entry_time 정렬** 기준 → N-트랜치에서 진입순 ≠ 청산순이라 "연속 손실"의 시간 의미가 흐려짐 ④ `policy_eval._winrate_breakeven` 의 승률 분모에 `pnl==0` 거래가 포함되나 wins/losses 에는 빠져 승률+패률 < 100% 가능 ⑤ `alpha_decomp` 의 `hasattr(c, "tz_localize")` 는 DataFrame 이 항상 갖는 속성이라 **항상 True 인 사문(死文)** | fresh-eyes Phase7 | 열림(LOW). 전부 **진단 지표·방어코드 수준**이라 D-046 판정에는 무영향. 해당 지표를 인용할 때 정의를 병기하고, ②는 θ>0 셀 재현 시 반드시 확인할 것. |
...
## 12-10. Phase 7 결과 (진행 중 — 진단 종결, 격자 구축 완료)
...
`policy_eval` **I-012 수정**. 결정 D-044~046, 이슈 I-014·015. 회귀 **414 passed / 1 skipped**.
...
**판정 기준 — 2단 바 (D-046, D-045 개정)**. 성과가 `net = 선택알파(실력) + 베타(시장) − 비용` 으로
분해되고 **연도별 net 부호는 대부분 베타가 결정**한다는 Phase 7 실측(§12-10)에 따라, "실력이 늘었나"와
"돈이 되나"를 **다른 시점에서 다른 축으로** 잰다. 배포 조건은 하나도 완화하지 않는다.

| | 바 | 재는 것 | 시점 |
|---|---|---|---|
| **진전 바** | ① **D2 baseline 초과**(ex-2024 +13.4bp·gross 61.6bp·MDD 33.3%) ② **연도별 선택 알파 전부 양수** ③ **지평 패밀리(3d/4d/5d) 선택 알파 부호 일치** | **우리 실력** | **Phase 7 종착** |
| **배포 바** | **연도별 net 전부 양수**(구 층1 바) + F-18 유효표본 보정 CI 병기 | **실제 수익성** | **Phase 8 관문3** |

- **구 층1 바는 폐기가 아니라 Phase 8 로 이동**한다. 지금은 **베타를 다룰 수단(헤지·국면필터)이 없는
  상태에서 베타 결과로 채점**하고 있어, 93셀 통과 0 의 실질적 이유일 수 있다. Phase 8 에서는 베타 대응이
  필수 구성요소이므로 그때 net 기준이 정당해진다.
- **지평 패밀리 요건도 net→선택알파로 교체**한다: 지평 길이 = **베타 노출량**(4h_4d 게이트봉 베타 +53.9bp
  vs 15m_6h +21.9bp, 2022 은 −18.2 vs +32.4)이므로 net 부호 불일치는 **과적합이 아니라 당연**일 수 있어
  두 원인을 구분하지 못한다.
- **D2 초과는 유지**하되, D2 의 gross 61.6bp 중 **약 4bp 는 신호가 아니라 배리어 기하학**(I-015, 7.4e 실측)
  임을 병기해 해석한다.
- **PARK 재소환 조건 재작성 필요**: §12-9 의 "15m 방향적중률 ≥60.4%" 는 **방향 예측 프레임 전제**라
  갈래 B(타이밍) 하에서 무효 — Step 7.5b 에서 프레임별로 다시 쓴다.
...
- **[신규] 베타 대응**(헤지·국면필터) — ~~알파는 양방향 실재하나~~ **I-014 정정 후: 방향 알파는 +6.4bp(2024 단독)뿐이고 2022 는 숏 0건이라 0.0**. 순노출 87% 롱(§12-9·12-10). **헤지는 방향 판단을 개선하지 못하므로 배포 바(D-046) 충족 수단으로만 검토**. 수수료 2배 비용 검증 필요.
