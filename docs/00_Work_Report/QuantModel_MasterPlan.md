...
>
> **현재 시점**: **Phase 0~5 완료**. **관문2(경제성) = FAIL-on-cost 유효**(§12-7 D-033). **Phase 5**(정책 최적화 → 지평 프론티어, §12-8)가 값싼 지렛대를 소진: 홀딩 정책 무효(D-034), 지평×배리어×x 프론티어에서 유일 양성이던 **4h/{3,4,5일}/x3도 fresh-eyes 독립검증 = 2024 단일연도 아티팩트·최근 OOS(2025–26) 음·비정상**(D-035, 코드결함 0). config 전수 원장·CARRY/PARK 처분(D-036, **완전 kill 0**). gate1 프록시(ll-margin·MCC)가 경제성 2회 오판·시간안정성 검정 교훈(I-006)·regime vol축 ex-post(I-005). **다음 = Phase 6 최적화3층**(정책 학습, CARRY 후보 위, **시간안정성 바 필수**) → Phase 7 현실실행·교차강건·관문3.
...
[Phase0] 검증하네스·국면태깅 (전 단계 관통) ─────────────────→ [Phase4] 멍청한 3층 (엔진 경유 경제성)
                                    [Phase5] 정책→지평 프론티어 (관문2 FAIL 유효, 4h=2024아티팩트)
                                                                → [Phase6] 최적화3층 → [Phase7] 현실·교차강건·관문3
```
...
| **6 최적화3층** | CARRY 후보 위 정책 학습(causal 국면필터·홀드·conviction·용량). 정책수준 held-out | **시간안정성**(ex-2024+최근OOS) | — |
| **7 현실·교차강건** | realistic execution(슬리피지·maker fill model)·멀티심볼(ETH/SOL)·재검증 | 배포가능성 | **관문3** |
...
| **D-036** | (Phase5 config 처분) **완전 kill 0, 2단**: CARRY(방향엣지 측정 → Phase6 정책검증, **전부 시간안정성 바** ex-2024+최근OOS held-out) / PARK(전기간 방향신호≈0 → 휴면·소환가능, 정책이 0엣지서 이익 못만듦·과적합기계 방지). **최종 discard는 최적화3층에서**(경제성=정책의존, dumb경제성=스크린 아닌 사형선고 아님). 은밀폐기 0·능동세트 유계(과적합방지 우선순위) | C | 3,5,8,19 | 확정 |
...
| **I-005** | (Phase5 fresh-eyes) 국면 태그 `tag_regimes` **vol축 = ex-post 전구간 quantile 문턱**(non-causal·분석전용). 트렌드축은 causal(trailing MA slope+forward 히스테리시스, 단 min_duration=14일 지연). **아티팩트 `regime` 열을 causal 매매필터로 그대로 쓰면 lookahead 주입.** | Phase5 프론티어 | **감사 완료**: 선별·경제성·결정은 전부 regime-독립(오염0, `model.fit`·summary 지표 regime무관). **미해결**: Phase6 국면필터 쓸 때 **causal 재유도 필수**(trailing/expanding quantile·트렌드 14일지연 감수). 기존 방화벽(D-003)의 연장. |
| **I-006** | (Phase5 fresh-eyes) **엣지 검정 방법론 결함** — 초기 베타/트렌드 반증이 **전체기간 base rate만** 사용해 **시간불안정성을 못 봄**(4h가 2024 아티팩트인데 "진짜 알파"로 오판). | Phase5 프론티어 | **교훈 박제**: 엣지 검정은 전체기간 아닌 **연도별 승률 vs breakeven·최근 OOS·ex-2024**로. Phase6 수용 바 = 시간안정성(held-out). 놀라운 양성일수록 fresh-eyes 독립검증(규칙17). |
...
- **은밀폐기 0.** 능동세트 유계 = 삭제 아닌 과적합방지 우선순위.

---

## 다음 단계

**Phase 5 종착 — 관문2 FAIL-on-cost 유효**(§12-8). 홀딩·지평·배리어·x·펀딩까지 소진했으나 **현실 비용을 견디는 시간안정 엣지 없음.** 유일 후보 4h는 2024 아티팩트(fresh-eyes 차단). Phase4 결론 보존·강화.

**다음 = Phase 6 (최적화3층)** — CARRY 후보 위 **정책 학습**으로 경제성을 살려본다(4h를 하드킬하지 않되 검증된 엣지로도 취급 안 함).
1. **지렛대**: ① **causal 국면필터**(I-005 — 트렌드축 causal 재구현, down 회피) ② holding/**펀딩-인지 청산** ③ **conviction 사이징** ④ **용량**(멀티슬롯/자본배치, 단일슬롯은 신호 13%만 포착) ⑤ 지평 앙상블(3·4·5일).
2. **규율**: **정책수준 train/val 분리**(관문2 최종셋 불가침)·F-1 사전등록·carry/park 원장 갱신.
3. **수용 바 = 시간안정성**: 정책-최적화 버전이 **ex-2024 양성 + 최근 OOS(2025–26) 무너지지 않음**을 **손대지 않은 held-out**으로 입증해야 승격. 못하면 discard(그때, 최적화3층에서).

**Phase 7 (현실·교차강건·관문3)**: realistic execution(슬리피지·**maker fill model**: SL=taker·비체결·adverse selection)·**멀티심볼(ETH/SOL) 교차강건**·실펀딩 1급·walk-forward 재검증 → **관문3 = 배포가능 net?**

**결정 트리**: Phase6 정책이 **시간안정 net**을 내나? {예→Phase7 현실검증→관문3 / 아니오→이 엣지 패밀리(삼중배리어 MLP on OHLCV) 경제성 사망 확정→새 엣지 방향(다른 피처·데이터·접근) 결정점}. **무한 정책×config 탐색 금지**(F-1) — CARRY 유계·held-out 판정.

**방법론 교훈(박제)**: ① **스크린(gate1·dumb경제성)은 사형선고 아님** — 경제성은 정책의존, 최종 discard는 최적화3층. ② **엣지 검정은 전체기간 base rate 아닌 시간안정성·최근OOS·연도별 승률vs breakeven**(I-006). ③ **놀라운 양성일수록 fresh-eyes 독립검증**(규칙 17이 4h 과적합 차단).
