...
- **I-BP001 (carry from I-B007 후속)**: funding_fee 백테 미반영 — ~~BP-1에서 fix 필수~~ → **BL 종착까지 carry-over (BL-1-5 미진입 확정 2026-05-05)**. 보유 시간 평균 1.6시간이라 영향 미미 추정. BL-2 §4.2.1 paper trading 시 펀딩률 실 비용 모니터링으로 재검증 필수.
...
BL-1 본 검증 (walkforward + Regime-matched) 모두 통과:
...
- ⚠️ walkforward + Regime 미수행 → I-BL002 carry (BL-2 진입 전 검증 권장)

**2순위**: dl_lstm (walkforward PF 4.12, Regime Calmar 187, MDD 4.0%)
...
|---|---|------|---|---|---|
| I-BP001 | Phase B-3 / E-2-2-FIX (carry from PATH_B_ML_STRATEGY §14.2) | `FeeModel.estimate_funding`이 `funding_enabled=True`라도 0 반환 (line 57-64). `engine_base.close_position`에 `funding_fee=0.0` 하드코드. config의 `accounting.funding_enabled=true`이지만 실제 백테에 미반영. 라이브 LONG 포지션 평균 -0.01%/8h 펀딩률 추가 비용 발생 가능 (보유 시간 짧으면 미미할 수 있음) | src/accounting/fee_model.py + src/core/engine_base.py + src/backtest/engine.py | ~~Phase BP-1~~ → ~~BL-1 §3.2.6~~ → **BL 종착까지 carry-over (BL-1-5 미진입 확정 2026-05-05)** | 미해결 — 보유 시간 평균 1.6시간으로 영향 미미 추정. **BL-2 §4.2.1 paper trading 시 펀딩률 실 비용 모니터링으로 재검증 필수** |
