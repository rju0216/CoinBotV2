...
작성 시점: 원작 2026-05-05 (BL-1 종착 + BL-2 재구성 직후) / 갱신 2026-05-09 (PATH_B_LIVE_TRADING 종착 반영, BLE-5/6 신설)
...
2. **BLE-5 (I-BP001 백테 통합)** — 작은 hotfix. 백테 정확성 보강. 라이브 펀딩률 모니터링 데이터 누적 후
3. **BLE-1 (다중 거래소)** — OKX 의존성 분산. 자금 안정성 강화
4. **BLE-6 (라이브-백테 정합성 검증)** — 라이브 거래 누적 (~1개월 / 거래 ≥ 30건 권장) 후 baseline 의미 ↑. 다른 BLE 진행 시 회귀 영향 측정 baseline 으로 재활용 가능
5. **BLE-4 (BP-1 데이터 carry)** — 데이터 인프라 준비 시. v005 학습 (사용자 GPU)
6. **BLE-3 (Survivorship)** — 다른 코인 학습. 가장 큰 작업 (사용자 GPU + 데이터)
...
PATH_B_LIVE_EXTENSION 모든 step (BLE-1 ~ BLE-6) 종착 시 "경로 B" 모든 작업 완료. 그 후:
- 다른 시간프레임 추가 (1m/5m 고빈도 또는 1h/4h 저빈도) — 별도 PATH 가능
- 새 모델 paradigm 탐색 (LLM 기반 등) — 새 PATH 시작 가능
- 운영 모니터링 자동화 (일일 리포트 텔레그램, OOS Decay 자동 모델 교체) — 운영 누적 후 필요성 재검토 시 별건 PATH 가능
