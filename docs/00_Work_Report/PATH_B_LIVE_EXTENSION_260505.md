...
| (carry) | └ I-BLE002 (예정, 1순위): BLE-6-1 sync 알고리즘 fix (2026-05-24 발견) | 미해결 | — | sync 0/16 매칭 실패 — 원인 A(amount=contracts) + B(reduceOnly 누락) + C(limit 100) + D(사용자 외 거래 섞임). 사용자 라이브 정지 시점에 즉시 진행 (AA=나 결정: I-BLE001 와 별건). 상세 plan §11 I-BLE002 entry 참조 |
| (carry) | └ I-BLE001 (예정, 2순위): 라이브 재시작 daily_pnl 복원 + close_at 컬럼 + sync 후 메모리 정정 | 미해결 | — | I-BLE002 fix 완료 + sync 정상 검증 후 라이브 정지 시점에 진행 (Z=가). 견적 ~95분. 상세 plan §11 I-BLE001 entry 참조 |
...
| I-BLE002 | BLE-6-1 sync 첫 trigger 시 발견 (2026-05-24 19:30) — Trade 16 close 후 `Trade sync: 0 synced, 16 failed` | **BLE-6-1 sync 알고리즘이 OKX 응답과 mismatch 로 전건 매칭 실패**. 4개 영역: **A. `amount` 단위 = contracts (정수/소수), BTC 아님** — `fetch_my_trades` 응답의 `amount` 가 OKX 의 `fillSz` (contracts). 본 시스템 contract_size=0.01 BTC. 시스템 db_size(BTC) vs OKX amount(contracts) 직접 비교 → 100배 차이로 tolerance 0.005 무관 fail. **B. `reduceOnly` 필드 OKX info 에 없음** (`reduceOnly_raw: null`, `info.reduceOnly: null`) — 대신 `info.fillPnl` 필드 사용 가능 (entry fill 은 fillPnl=0, close fill 은 fillPnl=실현PnL). 현재 알고리즘은 모든 fill 을 `reduce_only=False` 로 인식 → exit 매칭 영역 전건 fail. **C. fetch_my_trades limit=200 요청해도 100 fills 만 반환** (OKX V5 `/api/v5/trade/fills` 한도) — 18일 데이터 + 다중 fill 큰 거래 (예: 24 fills 1 order) 시 잘림. pagination 필요. **D. 사용자 외 거래 섞임** — DB Trade 1 (7.5 contracts) 외 다른 거래 (2.35 contracts 등) 가 응답에 포함. 매칭 자연 skip 으로 처리 (영향 없음, 정보용) | src/live/trade_sync.py + tests/test_trade_sync.py + scripts/diagnose_okx_fills.py (이미 commit) | **BLE-6-1 보강 fix (예정, 2026-05-24 발견 즉시 결정)** | 미해결 — 결정 사안 확정 (AA=나: I-BLE001 와 분리 진행 / AB=가: pagination 포함). 진행 트리거: **사용자 라이브 정지 시점 즉시** (다음 close 대기 X — 진단 완료라 즉시 작업 가능). **상세 5-step 계획**: ① `trade_sync.py` `_group_and_aggregate` 수정 — `total_amount_contracts × contract_size` 변환 후 BTC 단위 저장. `info.fillPnl` 필드 추정 (`float(fillPnl) != 0` → reduce_only). contract_size 인자 추가 — ~25분. ② `sync_all_unsynced` 시그니처에 `contract_size` 인자 추가 + `_close_with_funding` 호출 시 `broker.executor.contract_size` 전달 — ~10분. ③ pagination 추가 — `fetch_my_trades` 호출을 *last fill ts 기반 다음 page* 반복 (`since=last_ts+1`) 또는 *N 시간 단위 since 분할* 까지 모든 fill 누적. 매 page 100 fill, 총 ~500 limit (안전망) — ~30분. ④ 단위 테스트 갱신 — `test_id_match_success` / `test_time_match_fallback_success` / `test_multi_fill_aggregate` 의 amount 단위 변환 + fillPnl 기반 reduce_only 검증. pagination 단위 테스트 1-2건 신규. fee_currency 테스트는 그대로 (M=가 결정 유지). 기존 6건 → 8건 → 회귀 481→483 pass — ~45분. ⑤ 작업 보고서 §10 갱신 + I-BLE002 ✅ + 사용자 라이브 재시작 안내 + 다음 close 시 sync 정상 검증 — ~10분. **견적 합 ~120분**. **진단 근거** (scripts/diagnose_okx_fills.py 실행 결과 2026-05-24): first fill `amount:1.45`,`info.subType:"1"`,`info.fillPnl:"0"`,`reduceOnly_raw:null`. last fill `amount:6.23`,`side:"sell"`,`info.fillPnl:"44.13332"`. 32 unique orders 중 `3542434205647822848`(24 fills, sell, total 7.5 contracts) = DB Trade 1 의 exit (×0.01 = 0.075 BTC 일치). **다음 작업 (I-BLE001) 진행 조건**: I-BLE002 fix 후 라이브 재시작 → 다음 close 시점 `Trade sync: N synced, M failed` 로그에서 *synced 가 미sync 수의 80%+* 확인 후 → I-BLE001 진행 (시점 사용자 결정) |

신규 carry-over 후보 ID는 I-BLE003~ 형태로 등록.
