...
| (carry) | └ I-BLE001 (예정): 라이브 재시작 daily_pnl 복원 + close_at 컬럼 + sync 후 메모리 정정 | 미해결 | — | 다음 close 후 BLE-6-1 sync 결과 검증 + 라이브 정지 시점에 진행 (Z=가). 견적 ~120분. 상세 plan §11 I-BLE001 entry 참조 |
...
신규 carry-over 후보 ID는 I-BLE002~ 형태로 등록.
