...
### 1.4 운영 권장 (BL-1 종착 시점, 2026-05-05 갱신)

BL-1 본 검증 (walkforward + Regime-matched) 모두 통과 — 라이브 채택 안전 강화.

**1순위 (안정성 우선)**: **`dl_transformer` v007 + isotonic** ⭐ (walkforward + Regime 일관 1위)
- BL-1-3 walkforward (26 folds × 4.5년): Calmar 74.3, MDD 5.68%, 26/26 positive
- BL-1-4 Regime (2020-2021 강세 bull): Calmar 200, MDD 3.72%
- **SHORT win 76.21%** (강세 bull regime에서도 정확) — 단순 long-bias 아님
- 두 평가 모두에서 1위 → **라이브 채택 최우선 후보**

**1순위 (다양성/단일 OOS 우수)**: **`ensemble` (4 모델 v007 + isotonic)**
- BP-3-2 단일 OOS 1118.51% / MDD 5.27% / Calmar 212.2
- walkforward + Regime 미수행 — I-BL002 carry (BL-2 진입 전 검증 권장)

**2순위**: dl_lstm (walkforward PF 4.12 1위, Regime Calmar 187)
**3순위**: ml_lightgbm/ml_xgboost (GBDT 통계적 동등, Regime Calmar 127-134)
...
#### 4.2.1 Fail-safe 강화 (BL-2-1, 코드)

**현재 상태**:
- `src/risk/manager.py`: DD락(35%), 일일 손실 한도(5%), EMERGENCY BRAKE 로그 ✅
- `src/execution/live_executor.py` `_retry_api` (MAX_RETRIES=3, RETRY_DELAY=1.0) ✅
- `src/live/oos_monitor.py` LiveOOSMonitor (alpha decay 자동 감지) ✅
- ❌ **알림 인프라 부재** (logger.warning만, 사용자가 로그 봐야 인지)
- ❌ **명시적 Circuit breaker 부재** (연속 실패 N회 자동 정지 미구현)

**작업**:
1. `src/utils/notifier.py` 신규 — 텔레그램/이메일 알림 helper
   - DD락 트리거, 일일 손실 한도 도달, OOS monitor 임계 미달, Circuit breaker 발동, 거래 진입/청산 등 이벤트 라우팅
   - config: `live.notifications.{enabled, telegram_token, telegram_chat_id, email_*, alert_levels}`
2. Circuit breaker 신규 — `LiveExecutor`에 연속 API 실패 카운터 + 임계 도달 시 raise (자동 정지)
3. RiskManager의 EMERGENCY BRAKE/일일 한도 도달 시 notifier 자동 호출 (event_bus 연계)
4. 단위 테스트 (notifier mock, circuit breaker)

#### 4.2.2 호가창 인프라 (BL-2-2, 코드 + 데이터, BL-1 §3.2.4 carry)

**작업**:
1. `src/data/orderbook.py` 신규 — OKX 호가창 snapshot 수집 (ccxt `fetch_order_book` 활용)
2. `src/execution/paper_executor.py` 확장 — `book_depth` 인자 추가, 사이즈 대비 호가 침투 시 실효 체결가 산정
3. `BacktestEngine` slippage_pct 대신 호가창 기반 시뮬 (옵션)
4. paper trading 시작 시 호가창 동시 수집 인프라

#### 4.2.3 Paper trading (BL-2-3, 사용자 운영 1주)

**작업**:
1. `python -m src.main paper --config config/<선택 모델>.yaml` 1주일 운영
2. 라이브 가격 + paper executor로 모델 신호 + Fail-safe + OOS monitor 통합 검증
3. **I-BP001 펀딩률 실 비용 모니터링** (라이브는 `_close_with_funding`로 자동 반영, 정량 추적)
4. 호가창 데이터 누적 (BL-2-2 인프라 활용)
5. (옵션) dl_transformer vs ensemble 비교 운영

#### 4.2.4 소액 실거래 점진 전환 (BL-2-4, 사용자 운영)

**작업**:
1. **소액 실거래 1주일** — 사안 E 결정 자금 비중 (default 1% 추천)
2. paper trading vs 실 거래 결과 차이 정량 (수익률, 거래 수, fees, 슬리피지)
3. **점진 확장** — 1주 안정 후 자금 비중 증가
...
| Step | 검증 기준 | 결과 |
|---|---|---|
| BL-2-1 Fail-safe | notifier 단위 테스트 + circuit breaker 인위적 실패 주입 정지/알림 확인 | 대기 |
| BL-2-2 호가창 | paper_executor book depth 단위 테스트 + 백테 호가창 시뮬 비교 | 대기 |
| BL-2-3 Paper trading | 백테 vs 실 결과 차이 (수익률, 거래 수, fees, 슬리피지). I-BP001 영향 정량 | 대기 |
| BL-2-4 소액 실거래 | 자금 안전 + 모델 신호 정확 + 점진 확장 안정성 | 대기 |
...
| (대기) | **Phase BL-2: 라이브 진입** (재구성) | 대기 | — | Fail-safe(BL-2-1) + 호가창(BL-2-2) + Paper(BL-2-3) + 소액 실거래(BL-2-4). 종착 = 라이브 거래 시작 |
