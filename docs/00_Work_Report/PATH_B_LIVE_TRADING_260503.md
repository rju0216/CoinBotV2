...
| BL-2-3 Paper trading 1일 | 코드 통합 흐름 + 운영 절차 + 치명적 버그 | ✅ 완료 (조기 종료, 2026-05-06, 467b00c) — paper 운영 + hotfix 7건 통합 (I-BL003~I-BL007). 핵심 6개 검증 충족 (DataFeed·봉 마감→신호·OOS warmup·호가창·sub-plugin voting·학습-추론 cycle 일관성). 거래 흐름은 횡보장으로 미발생 → 단위 테스트 우회 검증 |
| BL-2-4 소액 실거래 | 자금 안전 + paper-실 거래 격차 정량 + 점진 확장 안정성 | ✅ 완료 (2026-05-09) — 라이브 운영 4일 + 거래 4건 (Trade 1~3 실거래 + Trade 4 manual close 시연) + hotfix 11건 통합 (I-BL008~I-BL018). hotfix-K e2e 검증으로 5 hotfix 동시 효과 확인 (외부 청산 자동 동기화·정확한 exit fetch·redundant skip·텔레그램 plain text·daily_pnl 자동 누적). 자금 점진 확장은 사용자 영역 (시작 자금 ~$3,370 유지, 향후 자율 결정) |

전체 회귀: 329 → **466 passed** (BL-2-1/2 + warm-up 신규 62 + BL-2-3 hotfix 18 + BL-2-4 hotfix 17 + hotfix-N daily reset 4 + 회귀 0)
...
- I-BP001 funding_fee 백테 통합 — BLE-5 (라이브 운영 펀딩률 모니터링 데이터 누적 후 처리. 보유 시간 평균 1.6h → 1건당 ~0.002% PnL 영향, 백테 정확성 보강)
...
| 2026-05-09 | **Phase BL-2: 라이브 진입** (재구성) | ✅ 완료 | (이번 커밋) | BL-2-1/2/3/4 모두 종착. 라이브 거래 시작 = 본 목적 달성. hotfix 18건 통합 (I-BL003~I-BL018). EXTENSION 으로 5개 carry (BLE-1~5) |
...
| 2026-05-09 | └ BL-2-4 hotfix-M: I-BL017 DEVELOPER_GUIDE 갱신 | ✅ 완료 | 515ab3b | 9 영역 갱신 (§3 ML+DL/§4.3 hook 행/§4.5 진단 컨벤션/§5.1 봉 마감/§5.3 청산/§5.4 재시작 복원 신규/§5.5 cycle 일관성 신규/§10 알림+거래소 패턴/§11.6 라이브 디버깅). 라인 420→544 (+124). 인용 클래스/함수 18개 grep 검증. 단위 회귀 459 pass 유지 (코드 변경 0). USER_GUIDE §6 cross-link 3곳 추가 |
| 2026-05-09 | └ BL-2-4 hotfix-L: I-BL016 _restore_state case 2 daily_pnl 누적 | ✅ 완료 | fc478c4 | `_fetch_actual_exit` 시그니처 4-튜플 확장 (`exit_ts_ms` 추가). case 2 same-day(UTC) 판정 후 `risk_manager.add_pnl`. B-3=나(EXIT 알림 skip, noise 회피) / B-4=나(update_equity 추가 호출 skip). 단위 3건 신규 + 5건 회귀 흡수(unpack 갱신) (459→462 pass) |
| 2026-05-09 | └ BL-2-4 hotfix-N: I-BL018 RiskManager 자동 daily reset | ✅ 완료 | a31127c | `last_reset_date` 필드 + `maybe_reset_for_new_day(now)` 메서드 추가 (UTC date 비교, naive datetime UTC 가정 처리, daily_pnl + _daily_loss_published 둘 다 reset). `evaluate_strategies_on_bar` 시작점 1줄 호출 — 백테/페이퍼/라이브 일관 적용. 단위 4건 신규 (TestDailyResetBoundary). C-1=다(백테 포함 + 노트). 회귀 462→466 pass. DD락·MDD·peak_equity 안전장치 영향 0 |
| 2026-05-09 | └ BL-2-4 종착 (라이브 거래 시작 안정성 입증) | ✅ 완료 | (이번 커밋) | 라이브 운영 4일 + 거래 4건 + hotfix 11건 통합 (I-BL008~I-BL018) + Trade 4 e2e 검증. critical 이슈 모두 해결. 향후 라이브 운영 누적·새 이슈는 BL-2-4 이후 hotfix 흐름 또는 EXTENSION 으로 처리 |
...
| I-BL016 | I-BL015 분석 중 발견 | `_restore_state` case 2(거래소 ∅ + DB O 재시작 시점)에서 DB는 close되지만 `risk_manager.daily_pnl`에 누적 안 됨 → daily_loss_limit 정확성 영향. 같은 날 재시작 vs 다른 날 재시작 정책 결정 필요 | src/live/engine.py | **BL-2-4 hotfix-L** | ✅ 해결 — `_fetch_actual_exit` 시그니처 4-튜플 확장(`exit_ts_ms` 추가, ccxt order timestamp 활용). case 2 에서 `now_utc.date()` vs `exit_dt.date()` UTC 비교로 same-day 판정 → same-day 시 `risk_manager.add_pnl(pnl)` + INFO 로그, different-day 시 skip + INFO 로그. fetch fallback (timestamp 부재) 은 보수적으로 different-day 가정. 결정 사안: B-1 좁은 범위(I-BL018 별건) / B-2 UTC 자정 / B-3 EXIT 알림 발송 skip(시작 시점 noise/지연 알림 혼란 회피) / B-4 update_equity 추가 호출 skip(라인 423 초기 호출 broker ground truth 충분). 단위 테스트 3건 신규(same-day/different-day/혼합) + 5건 회귀 흡수(unpack 시그니처 갱신, 459→462 pass) |
...
| I-BL018 | BL-2-4 hotfix-L 분석 중 발견 (`reset_daily_pnl` grep 결과) | `RiskManager.reset_daily_pnl()` 정의되어 있으나 `src/` 안 호출처 없음. 라이브 운영 24시간 이상 시 `daily_pnl` 이 자정에 reset 안 되고 영구 누적 → `daily_loss_limit` 의 "daily" 의미 상실. 첫 거래 큰 손실 후 다음 날에도 한도 트리거 유지 → 정상 진입까지 차단 가능. I-BL016 fix(case 2 same-day add_pnl) 와 묶기엔 변경 영역 결이 다름(case 2 정리 vs 자정 timer/이벤트 도입) | src/risk/manager.py + src/core/engine_base.py | **BL-2-4 hotfix-N** | ✅ 해결 — `RiskManager` 에 `last_reset_date` 필드 + `maybe_reset_for_new_day(now)` 메서드 추가 (캡슐화: RiskManager 가 day boundary 자체 관리). `AbstractEngine.evaluate_strategies_on_bar` 시작점에서 1줄 호출 → 백테/페이퍼/라이브 단일 entry 일관 적용 (CLAUDE.md 라이브-백테 일관성 원칙). UTC date 비교, naive datetime UTC 가정 처리. 첫 호출은 base date 만 설정 (no-op). 결정 사안: C-1=다(백테 포함 + 노트) / C-2=가(첫 호출 base) / C-3=가(UTC) / C-4=가(단위+회귀). 단위 4건 신규(TestDailyResetBoundary), 회귀 462→466 pass. **백테 결과 영향 노트**: 이전 백테 측정값(예: BL-1-3 walkforward Calmar/MDD, BL-1-4 regime 결과 등)은 `daily_loss_limit` 의 *영구 누적* 동작 기반으로 측정. hotfix-N 적용 후 동일 config·동일 데이터로 재실행 시 매일 한도 reset 으로 거래 패턴 변화 → PnL/MDD/Calmar 등 결과 *수치*가 달라질 수 있음. 단 MDD 측정 메커니즘 자체와 DD락(peak_equity 기반)·max_position_size·max_concurrent_positions 안전장치는 영향 0 |

신규 carry-over 후보 ID는 I-BL019~ 형태로 등록.
...
### BL-2-4 종착 (2026-05-09) — 라이브 거래 시작 안정성 입증
...
- daily_loss_limit 정확성 보강: hotfix-L (I-BL016, _restore_state case 2 same-day(UTC) daily_pnl 누적)
- 자정 경계 자동 reset: hotfix-N (I-BL018, RiskManager.maybe_reset_for_new_day, 백테/페이퍼/라이브 일관 적용)

**Carry-over (EXTENSION 으로)**:
- I-BP001 (funding_fee 백테) → BLE-5 (라이브 운영 펀딩률 모니터링 데이터 누적 후 처리)
- I-BL002 (ensemble walkforward) → BLE-2 (BL-1-3 종착 시 결정)

**종착 근거** (권장 기준 1-2주 / 거래 ≥ 20-30건 미달이지만 종착 결정):
- critical 이슈 모두 해결 (I-BL008~I-BL018, 11건 hotfix 통합)
- hotfix-K 라이브 e2e 검증 통과 — Trade 4 시연으로 5 hotfix 동시 효과 입증 (외부 청산 자동 동기화·정확한 exit fetch·redundant skip·텔레그램 plain text·daily_pnl 자동 누적)
- 라이브 운영 인프라 자체 안정성 입증 (외부 청산 자동 동기화·daily reset 자동·재시작 복원 4 case·SL/TP conditional order 검증/재등록)
- 향후 라이브 운영 누적·새 이슈는 BL-2-4 이후 hotfix 흐름 또는 EXTENSION 단계에서 처리

### PATH_B_LIVE_TRADING 종착 (2026-05-09)

**본 목적 달성**: 라이브 거래 시작 = 경로 B 본 목적 충족.

**종착 요약**:
- BL-1 (Regime + 추가 검증) ✅ 완료 (2026-05-05) — 4 모델 26/26 positive walkforward, 강세 bull regime robust, dl_transformer 일관 1위
- BL-2-1 Fail-safe ✅ / BL-2-2 호가창 ✅ / OOS warm-up ✅ (2026-05-05)
- BL-2-3 Paper trading ✅ (2026-05-06, 조기 종료) — hotfix 7건 통합
- BL-2-4 라이브 거래 시작 ✅ (2026-05-09) — hotfix 11건 통합 + e2e 검증

**EXTENSION carry-over** (PATH_B_LIVE_EXTENSION 으로 분리):
- BLE-1: 다중 거래소 (Binance + OKX)
- BLE-2: Ensemble walkforward (I-BL002 해결)
- BLE-3: Survivorship bias (다른 코인 학습)
- BLE-4: BP-1 데이터 확장 carry (펀딩률·OI + 2020 이전, 인프라 준비 시)
- BLE-5: I-BP001 funding_fee 백테 통합 (라이브 운영 펀딩률 모니터링 후)

**다음 진행 후보** (라이브 안정화 누적 후):
- **PATH_B_LIVE_EXTENSION** — BLE-1~5 본격 처리
