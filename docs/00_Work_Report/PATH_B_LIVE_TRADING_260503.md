...
- 펀딩률·OI 피처 통합 (BP-1)
- I-BP001 (funding_fee 백테) + I-BP002 (백테 warmup) fix
- Live OOS monitoring + 동적 사이징 + 백테 warmup 자동화 (BP-2)
- Calibration 본격 적용 (config 기본 isotonic) + Ensemble plugin (BP-3)
- 5개 모델 v006 (BP-1 데이터 확장 + BP-3 Triple-barrier label)

### 1.2 잠재 이슈 트래커 carry-over

PATH_B_PRODUCTION 작업 중 발견된 이슈 중 BL에서 처리할 영역. (BP 종착 시 갱신 예정)
...
| # | 단계 | 상태 | 커밋 | 비고 |
|---|------|------|------|------|
| (대기) | Phase BL-1: Regime + 추가 검증 | 대기 | — | Regime-matched + Walk-forward 통합 OOS + Lookahead 추가 점검 + 호가창 + Multi-hypothesis |
...
PATH_B_PRODUCTION 종착 시 carry-over 후보 ID는 I-BL001~ 형태로 등록.
