...
| I-BL013 | I-BL010 분석 중 발견 (DB recovery 부정확) | `_restore_state` case 2(거래소 ∅ + DB O)가 모든 trade를 exit_price=entry_price, pnl=0, ENGINE_SHUTDOWN으로 close. 실제 SL 청산 손실(예: -$40.85)이 DB에 누락되어 통계/분석 부정확 | src/live/engine.py | **BL-2-4 hotfix-H (Step 3)** | ✅ 해결 — `_fetch_actual_exit` helper 추가. ccxt `fetch_my_trades`로 실제 청산 정보(exit_price, pnl, reason) 복원. fetch 실패 시 SL 가격 + 단순 PnL fallback + WARNING (사용자 수동 update 권장). 단위 테스트 3건 |

신규 carry-over 후보 ID는 I-BL014~ 형태로 등록.
