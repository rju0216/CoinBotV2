...
| 2026-05-09 | └ BL-2-4 hotfix-L: I-BL016 _restore_state case 2 daily_pnl 누적 | ✅ 완료 | (이번 커밋) | `_fetch_actual_exit` 시그니처 4-튜플 확장 (`exit_ts_ms` 추가). case 2 same-day(UTC) 판정 후 `risk_manager.add_pnl`. B-3=나(EXIT 알림 skip, noise 회피) / B-4=나(update_equity 추가 호출 skip). 단위 3건 신규 + 5건 회귀 흡수(unpack 갱신) (459→462 pass) |
| 진행 중 | └ BL-2-4 라이브 운영 (사용자) | 진행 중 | — | 자금 점진 확장은 사용자 자율. 1-2주 안정 후 BL-2-4 종착 결정 |
| (carry) | └ I-BL018: daily_pnl reset 메커니즘 부재 | 미해결 | — | hotfix-L 분석 중 발견. `RiskManager.reset_daily_pnl()` 정의됐으나 호출처 없음 → 라이브 24h+ 운영 시 daily_loss_limit "daily" 의미 상실 가능. 별건 hotfix 처리 예정 |
...
| I-BL018 (carry) | BL-2-4 hotfix-L 분석 중 발견 (`reset_daily_pnl` grep 결과) | `RiskManager.reset_daily_pnl()` 정의되어 있으나 `src/` 안 호출처 없음. 라이브 운영 24시간 이상 시 `daily_pnl` 이 자정에 reset 안 되고 영구 누적 → `daily_loss_limit` 의 "daily" 의미 상실. 첫 거래 큰 손실 후 다음 날에도 한도 트리거 유지 → 정상 진입까지 차단 가능. I-BL016 fix(case 2 same-day add_pnl) 와 묶기엔 변경 영역 결이 다름(case 2 정리 vs 자정 timer/이벤트 도입) | src/live/engine.py + src/risk/manager.py | **BL-2-4 hotfix-N (예정)** | 미해결 (carry-over) — 처리 후보: ① `_on_bar_closed` 안에서 master_tf 봉 마감마다 UTC 자정 경계 인식 → reset / ② 별도 daily timer 도입. 정책 결정 필요(자정 기준 UTC vs KST). 라이브 영향이 I-BL016 보다 클 가능성이라 우선순위 중간~높음 |
...
**잠재 이슈 carry-over**:
- I-BL018 (신규): `RiskManager.reset_daily_pnl` 호출처 부재 → 라이브 24h+ 운영 시 daily_pnl 영구 누적 가능. 별건 hotfix-N (예정)
