...
| — | Phase B-3: PPO | 대기 | — | RL 접근 (직접 거래 결정). ml/env_trading.py(Gym 환경), scripts/train_ppo.py, plugins/rl_ppo.py, config/rl_ppo.yaml, tests/test_rl_ppo.py 신규 예정. **§11-#1 수정 적용 필수**: env observation_space와 `_get_obs()`를 1D flatten (SB3 MlpPolicy는 1D 기대). 엔진 hook(should_force_exit) 활용해 RL 정책의 직접 청산. SL/TP는 안전장치로만 (atr_sl_mult=5.0, RR=10.0). sequence_utils 재사용 |
| — | Phase B-3: PPO | 대기 | — | |
| — | Phase E: 통합 평가 | 대기 | — | |
