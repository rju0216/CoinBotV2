...
| 10 | Phase E-2-2-OPT: 백테 엔진 성능 최적화 | 대기 (E-2-2 종착 후 즉시 착수) | — | **배경**: E-2-2 백테가 **약 78시간 소요** (예상 6시간의 13배). E-2-3은 분할 1·4 × 5모델 × 4슬리피지 + calibration = **80+개 추가 백테** 필요 — 최적화 선행 필수. **목표**: 13-24배 단축 (78h → 3-6h). **시나리오 A** (전부 §13에서, 일괄): Step 0(Profiling 1h) + Step 1(features 사전계산 3-4h) + Step 2(모델 캐싱 1.5h) + Step 3(멀티프로세싱 2-3h) = **총 8-10h**. **상세 계획**: ①Step 1: BacktestEngine.initialize에서 전체 OOS의 features 1회 계산 → 매 봉 인덱스 슬라이스로 lookup (현재는 매 봉 compute_multi_tf_features 재계산 → 4년 OOS PPO 11.2M indicator 계산이 진짜 bottleneck). 변경: src/core/types.py(StrategyContext.precomputed_features 필드 추가), engine.py, 5개 plugin (None fallback으로 라이브 호환). 단독 효과 3-5x. ②Step 2: 5개 plugin에 module level `_MODEL_CACHE` dict 추가 — 같은 model_path 재로드 방지. 단독 효과 1.1x (PPO 7.7MB 큼). ③Step 3: multiprocessing.Pool(N=4) 워커로 30 백테 병렬. pickle 호환 OK (BacktestSpec dataclass), logger PID prefix, GPU 동시성 없음(CPU 추론). 단독 효과 4x. **메모리 분석**: 4 병렬 시 ~480MB (안전). **결과 일치성 검증**: 최적화 전후 metrics.json (total_trades / win_rate / total_return_pct / MDD) ±0.01% 일치. **선행 조건**: E-2-2 종착 ✅ (이번 커밋 후 즉시 착수 가능) |
| — | Phase E-2-3: 슬리피지 sensitivity + Calibration | 대기 (E-2-2-OPT 종착 후) | — | 슬리피지 (I-B010): 분할 1, 4 양 끝점에 5모델 × 4슬리피지 = 40개 백테. Calibration (I-B009): v001만 4개 분류 모델에 Platt scaling 적용 + 백테 4개. **E-2-2-OPT 단축 효과로 80+개 백테가 ~5h 내 완료 가능** |
