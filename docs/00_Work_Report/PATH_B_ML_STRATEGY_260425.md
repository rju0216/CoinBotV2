...
| — | Phase E-2-4: baseline 재실행 + 통계 + 시각화 + 종합 | 대기 | — | **사용자 결정으로 baseline 재실행 포함**. ①분할 A/B/Exp2/Exp3 × 5 모델 = 20 백테 재실행 (E-2-2-FIX 코드로 정확). ②Sharpe/Calmar Ratio 계산 (30 specs). ③부트스트랩 유의성 검정 (n=10,000). ④equity_overlay.png 시각화. ⑤종합 비교 표 CSV (분할별 데이터 양 효과 #4 재검증) |
