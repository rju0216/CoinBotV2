...
| — | Phase B-2b: Transformer | 대기 | — | B-2a 진입 시 패턴: B-2a 복사 + TransformerClassifier API. **참고**: LSTM이 GBDT와 동률인 한계 분석(§13 행 4 비고 참조) — Transformer도 같은 한계 부딪힐 가능성 높음 (같은 데이터·같은 피처 정보량). Attention의 다른 inductive bias로 미세한 차이 가능성은 있음 |
| — | Phase B-3: PPO | 대기 | — | |
| — | Phase E: 통합 평가 | 대기 | — | |
