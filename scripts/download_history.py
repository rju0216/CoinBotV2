"""과거 캔들 데이터 다운로드 유틸 (동기).

Usage:
    python scripts/download_history.py --timeframe 4h --start 2024-01-01 --end 2024-12-31
    python scripts/download_history.py --timeframe 1d,4h,15m --start 2020-01-01 --end 2026-01-01
    python scripts/download_history.py --timeframe 4h --limit 1000   # 최근 N개
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--symbol", help="심볼 오버라이드 (미지정 시 config 의 exchange.symbol 사용)"
    )
    parser.add_argument("--start", help="시작일 (YYYY-MM-DD). --limit와 상호 배타.")
    parser.add_argument("--end", help="종료일 (YYYY-MM-DD). --limit와 상호 배타.")
    parser.add_argument(
        "--limit", type=int, help="최근 N개 다운로드. --start/--end와 상호 배타."
    )
    ...
    try:
        for tf in timeframes:
            if args.limit:
                path = loader.download_to_csv(tf, args.limit)
