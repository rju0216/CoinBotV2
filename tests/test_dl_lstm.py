"""Phase B-2a: LSTM 전략 플러그인 단위 테스트.
...
class TestDLLSTMRegistration:
    def test_plugin_registered(self):
        """@register_strategy 데코레이터로 dl_lstm이 등록됨."""
        from src.strategy.plugins.dl_lstm import DLLSTM
        from src.strategy.registry import register_strategy

        register_strategy(DLLSTM)
        strategy_cls = get_strategy_class("dl_lstm")
        assert strategy_cls is DLLSTM
        assert strategy_cls.name == "dl_lstm"

    def test_default_timeframe(self):
        from src.strategy.plugins.dl_lstm import DLLSTM

        s = DLLSTM({"risk_per_trade_pct": 0.01, "max_leverage": 5})
...
class TestTrainScriptImport:
    def test_script_importable(self):
        """train_lstm.py가 import 가능한지 확인 (구문 오류 검증)."""
