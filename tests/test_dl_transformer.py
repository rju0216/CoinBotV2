"""Phase B-2b: Transformer 전략 플러그인 단위 테스트.

플러그인 등록, config 파라미터 처리(lookback 포함), SL/TP 계산을 검증.
모델 추론 테스트는 학습된 모델 + scaler가 필요하므로 별도 분리.
"""
...
class TestDLTransformerRegistration:
    def test_plugin_registered(self):
        """@register_strategy 데코레이터로 dl_transformer가 등록됨."""
        from src.strategy.plugins.dl_transformer import DLTransformer
        from src.strategy.registry import register_strategy

        register_strategy(DLTransformer)
        strategy_cls = get_strategy_class("dl_transformer")
        assert strategy_cls is DLTransformer
        assert strategy_cls.name == "dl_transformer"

    def test_default_timeframe(self):
        from src.strategy.plugins.dl_transformer import DLTransformer

        s = DLTransformer({"risk_per_trade_pct": 0.01, "max_leverage": 5})
...
class TestDLTransformerSLTP:
    ...
    def test_stop_loss_fallback_short_data(self):
        ...
        sl = s.compute_stop_loss(ctx, signal)
        assert sl < 67000.0
        assert sl > 67000.0 * 0.99


class TestTrainScriptImport:
    def test_script_importable(self):
        """train_transformer.py가 import 가능한지 확인 (구문 오류 검증)."""
