"""Phase B-1b: XGBoost 전략 플러그인 단위 테스트.

플러그인 등록, config 파라미터 처리, SL/TP 계산을 검증.
모델 추론 테스트는 xgboost 설치 + 학습된 모델이 필요하므로 별도 분리.
"""
...
class TestMLXGBoostRegistration:
    def test_plugin_registered(self):
        """@register_strategy 데코레이터로 ml_xgboost가 등록됨."""
        from src.strategy.plugins.ml_xgboost import MLXGBoost
        from src.strategy.registry import register_strategy

        register_strategy(MLXGBoost)
        strategy_cls = get_strategy_class("ml_xgboost")
        assert strategy_cls is MLXGBoost
        assert strategy_cls.name == "ml_xgboost"

    def test_default_timeframe(self):
        from src.strategy.plugins.ml_xgboost import MLXGBoost

        s = MLXGBoost({"risk_per_trade_pct": 0.01, "max_leverage": 5})
...
class TestTrainScriptImport:
    def test_script_importable(self):
        """train_xgboost.py가 import 가능한지 확인 (구문 오류 검증)."""
